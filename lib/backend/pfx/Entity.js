...
export default class PfxEntity {
    ...
    constructor(deps = {}) {
        ...
        this.config = deps.config || require('../../config').secret;
    }
    ...
    save(input) {
        ...
        return adapter.read(key)
            .then((output) => {
                ...
                if (!token.id && input._taxPayer.length) token.id = input._taxPayer;
                ...
                if (position === null) {
                    let cipher = crypto.createCipher('aes192', input.hash.substr(0, 8) + this.config.keyCert);
