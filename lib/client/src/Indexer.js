...
class Indexer {
    ...
    run(data, next) {
        ...
        if (this.data && this.data.length) {
            logger.info(`Data to index ${data.length}`);
            this._initQueue();
        } else {
            this._finish();
        }
    ...
    _client(index) {
        ...
        let runner = (meta) => {
            ...
            request.setHeader('Content-Length', Buffer.byteLength('mofos!'));
//            request.write(JSON.stringify(meta.data));
//            request.write('what a fak');
            request.end('mofos!');
        };
        let task = {
            meta: {
