...
if (argv.h) {
    console.log("usage:\n" +
        ...
        "-limit (optional) Simultaneous request limit, default 1\n" +
...
let _process = (data) => {
    if (data && data.length) {
        logger.info(`Processing... after run`);
        indexer.run(data, _indexed);
    } else {
        logger.info(`Nothing to process, after run`);
    }
}
...
let _indexed = () => {
    logger.info('All data was indexed');
    keeper.setBatch([]);
    isLocked = false;
}

let _task = setInterval(() => {
    if (!isLocked) {
        isLocked = true;
        logger.info('Staring new process...');
        run();
    } else  {
        logger.info('Locked waiting indexer...');
    }
}, 2000);